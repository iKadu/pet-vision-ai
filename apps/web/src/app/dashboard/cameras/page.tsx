...
export default function CamerasPage() {
  ...
  async function changeSource(nextSource: CameraType, sourceOverride?: string) {
    const sourceValue = sourceOverride ?? (nextSource === "webcam" ? "0" : nextSource === "screen" ? "screen" : rtspUrl.trim());
  ...
  async function useSavedCamera(camera: { id: string; type: string; source: string }) {
    ...
    if (await changeSource(cameraType, camera.source)) setActiveCameraId(camera.id);
  }
